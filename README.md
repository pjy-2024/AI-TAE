...
1. **不是"调 LLM 写 pytest"的 demo**：LLM 产出要过两道本地门禁（JSON 结构 + ast 静态校验），
   不合格带精确错误回传、限次重写——可执行率 100% 有 junit 留档可查；
2. **换被测项目 = 核心零改动**：被测特定面全部收敛在「适配契约」，第二个真实项目只写约 30 行
   适配器即接入并跑通（todo_app：password 认证 19 接口；fastapi_crud_todo：无认证 6 接口）；
3. **有工程判断而非堆功能**：方案 B「与其让 LLM 少犯错，不如让它没机会犯错」——资源改由框架
   fixture 创建，通过率 66.7% → 50%（仅加规则）→ 100%（方案 B），三轮真实对比；
4. **成本与护栏意识**：KV→RAG→LLM 分层（能本地绝不问模型）+ 语义层限次重试防烧钱；
   V2 自愈只修「定位失败」一类、AI 建议必须真实浏览器验证通过才采纳（自动化懂边界）；
5. **真实数字纪律**：连失败的实验都记录（加规则反而降），一切数字可复现、可回溯，无编造。
